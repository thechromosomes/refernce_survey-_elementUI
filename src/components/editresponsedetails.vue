<template>
    <v-layout wrap class="conatiner_responceform" v-if="id && (userrole.indexOf('1')>=0 || userrole.indexOf('2')>=0 || userrole.indexOf('3')>=0)">
      ...
      <div class="reportheader tabnodesign"> 
        <v-tabs
            class="header_navbar"
            centered
            v-model="activetab"
            >
            <v-tab key=0 @click="gotoresponselist(0)" >
      ...
      <v-container class="conatiner_dashboard" v-if="surveydata != null && surveyconfig != null">
        <div class="surveydetails">
          ...
          <v-layout class="surveyressect" >
            <v-flex xs12 sm6>
              <div>
                Export:-  <a target="_blank" :href="getexportlink('csv', attemptid)"><span class="exportbutton">CSV Export</span></a>
                  <a target="_blank" @click="exportPDFAllresponse"><span class="exportbutton">PDF Export</span></a>
      ...
      <v-container>   
                <button class="btn-custom1" @click="finishSurvey">Save</button>
      </v-container>   
...
import finalqestion from './surveyresponse/question/finalquestion'
...
import jsPDF from 'jspdf';
  ...
  methods: {
    ...
    gotoresponselist(page){
      if(page == 0){
        this.$router.push('/responselist/'+this.id)
      }
    },
    haveaccesstonav(){
    ...
    exportPDFAllresponse(){
      ...
      var htmlcontent = "";
      axios.get("/downlaodresponsexls?respid="+this.attemptid+"&onlydata=true",{}).then(response => {
