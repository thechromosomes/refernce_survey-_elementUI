    ...
    <div class="padding20">
        <v-layout row wrap  class="survey_lists" v-if="activedashboard == 1">
            <template v-for="(survey, surveyindex)  in surveys"  >
                <v-flex :key="`surveyid${surveyindex+''+survey._id}`" xs12 class="survey_list" v-if="((searchsurvey == '' || survey.surveyname.toLowerCase().indexOf(searchsurvey.toLowerCase()) != -1)) && ((searchsemester.length > 0 && searchsemester.indexOf(survey.semester) != -1 ) ||(searchsemester.length == 0))" >
        ...
        <v-flex>
            ...
            <table>
                ...
                <template v-for="(survey, surveyindex)  in surveys">
                <tr v-if="(searchsurvey == '' || survey.surveyname.toLowerCase().indexOf(searchsurvey.toLowerCase()) != -1)" :key="survey._id">
...
export default {
    ...
    methods:{
        ...
        getallsurveys(){
            if(this.project != ""){
                ...
                axios.post(`/findSurveys`,{id:this.project,lastdate:this.lastupdated}).then(response => {
    ...
    watch:{
        ...
        project: function (new_activesurveytab, old_activesurveytab) {
