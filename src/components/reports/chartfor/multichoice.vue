                ...
                if(response.data == null){
                    ...
                    response.data.forEach(answer => {
                        if(this.contactuserid.length > 0 && this.contactuserid.indexOf(answer.userid)<0){
                            ...
                            var only_as = []
                            if(answer.answer){
                                only_as = JSON.parse(answer.answer);
                            }
                    ...
                    all_choice.forEach(choice => {
                        ...
                        const total = all_answer.filter(a => a == choice._id);
                        totalanswer += total.length
                    ...
                    if(this.questiondata.other){
                        const total = all_answer.filter(a => a == 'other');
                        totalanswer += total.length
                        col_categorydata[col_categorydata.length] = {
                            name : 'Other',
                            y : total.length
                        }
