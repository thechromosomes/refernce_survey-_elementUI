         ...
         <v-container>
              <div class="ratingstar">
                <table class="table charttable">
                    ...
                    <template v-for="(sdata, index) in ratingstar" >
...
export default {
    ...
    return {
        ...
        "hover": {"animationDuration": 0},"animation": {"onComplete": function() {var chartInstance = this.chart,ctx = chartInstance.ctx;ctx.textAlign = 'center';ctx.textBaseline = 'bottom';this.data.datasets.forEach(function(dataset, i) {var meta = chartInstance.controller.getDatasetMeta(i);meta.data.forEach(function(bar, index) {var data = dataset.data[index]+"%";ctx.fillText(data, bar._model.x+20, bar._model.y + 7);});});}},maintainAspectRatio: false,legend: {display: false},tooltips: {"enabled": false},
            scales: {
                xAxes: [
                    {ticks: {
                        beginAtZero: true,
        ...
        getfilterdata:function(){
            ...
            axios.post(`/allanswers`,{'questionid':this.question._id}).then(response => {
                if(response.data == null){
                    ...
                    var all_answer = [];
                    const maxrate = this.question.maxrate
