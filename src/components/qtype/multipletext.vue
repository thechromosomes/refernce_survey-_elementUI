  ...
  <div class="text_multiple qtype">
    ...
    <div class="questionimage" v-if="qtypedata.haveimage == '1'">
      ...
      <div v-for="(answer, index) in qtypedata.choice" v-bind:key="index" class="tobedelete">
        <v-btn class="btn-extrasmall" fab  @click="deleteTextbox(index);"><img class ="deletequestion" src="../../assets/icons/trash-alt-light.svg" /></v-btn>
      ...
      <div class="que_bottom_btns">
        <v-chip class="addchip" @click="addTextbox();"><v-avatar><v-icon>add</v-icon></v-avatar>Add Textbox</v-chip>
...
export default {
  ...
  methods: {
    ...
    deleteTextbox(ele_index){
        ...
        axios.post(`/deleteoptions`,{'optionid':optionid}).then(response => {
          if(response.data.n==1){
            ...
            this.$emit("deleteTextbox",this.options);
          }
        }).catch(e => {
          this.errors.push(e)
        })        
    ...
    changeQuestion(currenttext){
      ...
      this.$emit("changeQuestion",this.questiontext);
    },
    ...
    addTextbox(){
        axios.post(`/addoptions`,{'questionid':this.id, 'choicestate':"0", 'choicetext':""}).then(response => {
          if(response.data.status==1){
            ...
            this.$emit("addTextbox",this.options);
          }
        }).catch(e => {
          this.errors.push(e)
        })        
