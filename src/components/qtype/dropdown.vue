<template>
  <div class="multichoice_question qtype">
    <div class="multichoice_option">
    ...
    <div v-for="(option, index) in qtypedata.choice" v-bind:key="index"  class="tobedelete">
      <v-btn class="btn-extrasmall" fab @click="deleteOptions(index)"><img class ="deletequestion" src="../../assets/icons/trash-alt-light.svg" /></v-btn>
      <v-layout align-center>
        <v-text-field :label="`Option ${index +1}`" @input="changeOption(index)" :key="index" v-model="option.choicetext"></v-text-field>
      </v-layout>
    </div>
      </div>
      <div class="que_bottom_btns">
        <v-chip class="addchip"  @click="addOption();"><v-avatar><v-icon>add</v-icon></v-avatar>Add option</v-chip>
...
export default {
  ...
  methods: {
    ...
    deleteOptions(ele_index, qtypedata_choice){
        ...
        axios.post(`/deleteoptions`,{'optionid':optionid}).then(response => {
          if(response.data.n==1){
            ...
            this.$emit("deleteOptions",this.options);
          }
        }).catch(e => {
          this.errors.push(e)
        })        
    ...
    addOption(){
        axios.post(`/addoptions`,{'questionid':this.id, 'choicestate':"0", 'choicetext':""}).then(response => {
          if(response.data.status==1){
            ...
            this.$emit("addOption",this.options);
          }
        }).catch(e => {
          this.errors.push(e)
        })        
    ...
    changeQuestion(currenttext){
      this.questiontext = currenttext;
    },
    updateQuestion(){
      var questionid = this.id
    ...
    changeOption(c_index){
      this.options[c_index].choicetext = event.target.value;
  ...
  watch: {
    ...
    this.debouncedUpdateOptions = _.debounce(this.updateallOptiontext, 500)
    this.checkrequired()
  },
  computed: {
    // a computed getter
