  ...
  <div class="multichoice_question qtype">
    ...
    <div v-for="(choice, index) in qtypedata.choice" v-bind:key="index" class="tobedelete">
      ...
      <div class="que_bottom_btns">
        <v-chip class="addchip" @click="addChoice();"><v-avatar><v-icon>add_circle_outline</v-icon></v-avatar>Add choice</v-chip>
...
<script>
var questionbuilder = require('../../models/questionbuilder.js');
export default {
  props:['qtypedata'],
  data() {
    return {
  ...
  methods: {
    ...
    updateHaveImage(newstate){
      console.log(newstate)
      if(questionbuilder.updateHaveImage(this.id, newstate)){
      console.log("Updated")
        this.qtypedata.haveimage=newstate
      } else {
        console.log("Failed")
      }
    },
