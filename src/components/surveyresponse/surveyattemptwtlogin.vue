...
import axios from 'axios';
  ...
  methods: {
    startNewattempt(){
        this.attemptemail =  localStorage.getItem("attemptemail");
        axios.post(`/attemptsurvey`,{'surveyid':this.id,'userid':this.currentuser,'semesterid':this.semesterid,'attemptemail':this.attemptemail}).then(response => {
    ...
    if(this.id == "" || this.id == undefined){
      ...
      axios.post(`/getsurvey`,{'surveyid':this.id}).then(response => {
        if(response.data == null){
          // console.log("invalid survey id:-"+this.id);
          // this.$router.push('/')
        } else {
