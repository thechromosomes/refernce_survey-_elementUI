<template>
    <v-layout v-if="rerender == 1 && surveydata" wrap class="conatiner_responceform paddingbottom50">
        ...
        <template v-if="surveyconfig">
          <v-container class="responceform" id="responceform" v-if="surveyconfig.display == '0' && havelogicjump==0">
              <div class="questionblock">
                  <div v-for="(question, index) in finalbuilderarray" :key="`questionid_${index}`" >
                      ...
                      <finalqestion @contactinfochnaged="ciChangecount++" :ciChangecount="ciChangecount"  :questiondata="question" :qno="getquestionno(index)" v-else-if="question.questiontext !== '' || question.questiontype == 'qtypeslider'" ></finalqestion>
                  </div>
              </div>
              <v-layout v-if="surveydata != null">
                ...
                <v-flex xs11 class="text-center">
                  <!-- recaptcha here -->
                  <div class="recaptchavalidation" v-if="surveyconfig.recaptcha == 1">
          ...
          <v-container v-else class="responceform" id="responceform">
            <div class="questionblock" v-if="is_submitted == false && currentquestiondata != null && readytodisplay == true">
                <mobileresponse @contactinfochnaged="ciChangecount++" :ciChangecount="ciChangecount"  :question="currentquestiondata" :qno="getquestionno(currentquestion)"></mobileresponse>
...
import Recaptcha from './recaptcha'
  export default {
  components: {finalqestion,displayheading,mobileresponse, Recaptcha},
   data() {
    return {
      id:null,
      rerender:1,
      ...
      builderarray : [],
      finalbuilderarray : [],
      questiontypes : ["multic","singlec","ratescal","ratestars","qtypeslider","qtypedatetime","dropdown","matrixquestion","singletext","multipletext","commenttext","numbertext","customform","emailtext","fileupload","ranking", "nps","contactInfo", "dataTable","thankyoupage"],
  ...
  methods: {
    ...
    finishSurvey(){
      if(this.mythankyou == ""){
        ...
        if(!this.recaptchaToken && this.surveyconfig.recaptcha == 1){
          this.errormessagebox = "please verify you are human (RECAPTCHA)"
          return;
        }
      }
      axios.post(`/checkallquestionattempt`,{'attemptid':this.attemptid, surveyid:this.id, token: this.recaptchaToken}).then(response => {
        if(response.data != "1" && this.surveyconfig.display == '0' && this.havelogicjump==0){
          this.errormessagebox = "Please answer all required question."
        } else {
          // to send the data to other one.gpex.com.au server
            let questions = this.choosedQuestion
            let userId = this.currentuser
            let errors = this.errors
            this.finalbuilderarray.filter(function (elem1) {
                questions.some(function (elem2) {
                    if(elem1.surveyid === elem2.survey && elem1._id === elem2.question_Id){
                      axios.post(`/senddatatoone`,
                        {
                          userid:userId,
                          semesterid:elem2.semester,
                          startdate:elem1.npsleft,
                          enddate:elem1.npsright
                        })
                      .then(response => {
                        console.log("one.gpex.com", response)
                      })
                      .catch(err => {
                        console.log("one.gpex.com", err)
                        errors.push(err)
                      })
                    }
              });
          ...
          //   axios.post(`/finishattempt`,{'attemptid':this.attemptid,token:"token", mythankyou:this.mythankyou}).then(response => {
          ...
          //     axios.post(`/finishattempt`,{'attemptid':that.attemptid,token:this.recaptchaToken, mythankyou:this.mythankyou}).then(response => {
          //       if(response.data != null){
    ...
    onlychangequestion(changeto){
      ...
      const new_question =  this.finalbuilderarray[changeto];
      this.currentquestiondata = this.finalbuilderarray[changeto];
      if(new_question.questiontype == "thankyoupage"){
        this.mythankyou = new_question._id;
        this.finishSurvey();
      } else {
        this.currentquestiondata = new_question;
    ...
    if(this.attemptid == "" || this.attemptid == undefined){
      ...
      axios.post(`/getattemptsurvey`,{'attemptid':this.attemptid}).then(response => {
        if(response.data == null){
          ...
          var aaaa = localStorage.getItem('loginuser');
        ...
        axios.post(`/getallwelcomethankyoudataforlogic`,{'surveyid':response.data.surveyid,datatype:'thankyoupage'}).then(response => {
          if(response.data == null){
          } else {
                ...
                this.builderarray.forEach(q => {
                  ...
                  if(q._id == element.questionid ){
                    if( q.qtype && q.qtype.length > 10 && q.questiontype == "dataTable") {
                      ...
                      // console.log("skipcontactinfo- ", skipcontactinfo)
                      // console.log("q.qtype- ", q.qtype)
                    ...
                    if(this.questiontypes.indexOf(q.questiontype)>=0 || (q.questiontype == 'descriptionbox' && q.required == "1")){
                      if((q.questiontext != "" || q.questiontype == 'qtypeslider') && !skipcontactinfo.includes(q._id)){
                        // console.log(q.questiontype);
                        if(q.questiontype == "contactInfo"){
                          // console.log("questiontype- ", q.questiontype);
                          skipcontactinfo.push(q._id  );
                        }
                        // if( q.questiontype == "dataTable" && contactinfo) {
                        //   q.contactinfo = contactinfo
                        //   contactinfo = null
                        ...
                        extras = [];
                      } else if((q.questiontype == 'descriptionbox' && q.required == "1") || q.questiontype == 'thankyoupage'){
                        q.extras = extras;
                        this.finalbuilderarray.push(q)
                      }
  ...
  mounted() {
    ...
    axios.post(`/getdatequestiontype`).then(response => {
      this.choosedQuestion = response.data.data
      })
    .catch(e => {
      this.errors.push(e)
    })
